import { useLanguage } from "@/i18n/LanguageContext";

const BookAppointment = () => {
  const bookingUrl = "https://magnifico.setmore.com/";
  const { t } = useLanguage();

  return (
    <div className="min-h-screen bg-background py-16">
      <div className="container mx-auto px-6">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-10 space-y-3">
            <p className="text-xs tracking-[0.35em] uppercase text-ember">{t("book.label")}</p>
            <h1 className="font-serif text-5xl md:text-6xl text-foreground">
              {t("book.title.a")} <em className="font-script">{t("book.title.em")}</em>
            </h1>
            <div className="w-12 h-px bg-ember mx-auto" />
            <p className="text-muted-foreground">{t("book.copy")}</p>
          </div>

          <div className="border border-border bg-card rounded-md overflow-hidden shadow-sm">
            <iframe
              src={bookingUrl}
              className="w-full h-[800px] border-0"
              title="MAGNIFICO Hair Salon booking"
            />
          </div>
        </div>
      </div>
    </div>
  );
};

export default BookAppointment;
